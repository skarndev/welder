...
src/welder/
  ...
  diag.hpp              welder::diag — ALL hand-rolled consteval diagnostics in one catalogue, each an exception struct carrying its canonical message as a `const char* what` default member init, THROWN during constant evaluation (C++26 constexpr exception → ONE compiler error with type + prose verbatim; supersedes the undefined-fn anchor idiom). Types: lang_out_of_mask_range / dangling_return_policy / bare_mark_only / no_matching_virtual_slot / stale_hook_signature. std-include-free (vocabulary may include it; annotations.hpp does). New diagnostics go HERE, not inline
  reflect.hpp           welded_for / policy_of / member_bound (throws diag::bare_mark_only on a bare `only`) / protected_welded / trusted_for / validate_return_policy (throws diag::dangling_return_policy) / public_bases — uses <meta>, includes diag.hpp
...
(containers, virtuals/directors) throws
