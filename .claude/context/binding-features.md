...
FIXED (2026-08-11) — the same blind spot, one level up: an overload group that
mixes a member DECLARED in the bound type with one FLATTENED IN from a base
(`WDL<V>::read(fs,key)` + inherited `ChunkedFile<WDL<V>>::read(span)`) used to
collapse onto ONE symbol and ONE lookup when both declaring scopes were
unspellable specializations — `index_of_named_member` counts within the DECLARING
scope, so both are index 0, and owner_expr falls back to the bound-type anchor for
both. (Loud, never silent: the duplicate-symbol #error fired.) Note the two do NOT
arrive in one add_method call — the flattened member is a separate group — so a
group-local dedup cannot see it. THE FIX is per-member and needs no interface
change: class_writer now carries `type_token` (symbol_token of the bound type, set
in open_class/open_nested_class where ^^T is in hand), and `member_scope_of<Fn>`
(emit/refs.hpp) compares it with symbol_token(parent_of(Fn)). Equal = declared
here → spelled exactly as before (zero golden movement). Different = FLATTENED →
the symbol is namespaced `_at_<scope-token>` and, when the scope has no spellable
name, the lookup goes through `wcs::base_scope(anchor, "<token>")` (reflect/
lookup.hpp: walks the public base chain derived-first for the scope whose
symbol_token matches, throws csharp_member_lookup_mismatch on drift). Applied at
all four sites (methods, static methods, fields, accessor pairs — the property
symbol is already keyed by the unique C# name, so only its owner changes). The
symbol namespacing also closes a second latent collision the note missed: a
