...
/** @file
    ...
    @note Overloaded methods: sol2 stores one value per name, so when a method name
    has several C++ overloads the last one registered wins (pybind11/nanobind chain
    them). Grouping overloads into a `sol::overload(…)` is a planned enhancement.
*/
#include <array>
#include <cstddef>
#include <meta>
...
struct backend {
    ...
    /** Bind member function @a Fn as a method (`obj:name(…)`). */
    template <std::meta::info Fn>
    static void add_method(auto& ut) {
        ut[std::define_static_string(std::meta::identifier_of(Fn))] = &[:Fn:];
    }
    ...
    /** Bind member operator @a Fn under its Lua metamethod (the specific overload
        is spliced, so unary/binary forms never collide). */
    template <std::meta::info Fn>
    static void add_operator(auto& ut) {
        ut[operator_mm(Fn).fn] = &[:Fn:];
    }
    ...
    static void add_function(module_type& m) {
        m[std::define_static_string(std::meta::identifier_of(Fn))] = &[:Fn:];
    }
