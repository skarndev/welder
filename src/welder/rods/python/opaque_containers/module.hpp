...
/** @def WELDER_OPAQUE_CONTAINERS_MAIN
    ...
    `welder_generate_opaque_containers` runs the generator with the output path.
...
#define WELDER_OPAQUE_CONTAINERS_MAIN(ns)                                        \
    int main(int argc, char** argv) {                                            \
        if (argc > 1) {                                                          \
            ...
            ::welder::rods::opaque_containers::rod::generate<^^ns>(welder_out_); \
        } else {                                                                 \
            ::welder::rods::opaque_containers::rod::generate<^^ns>(::std::cout);  \
        }                                                                        \
        return 0;                                                                \
    }
