#pragma once
//
// Backend-agnostic module entry point.
//
...
//
// (pybind11 and nanobind both emit PyInit_<name>, so those two can't coexist in
// one module — pick one Python backend.)
//
// The namespace token doubles as the module name: `geometry` is both ^^geometry
// (the bound namespace) and the PyInit_geometry / luaopen_geometry symbol, so
// there is no separate name argument. The trailing { } is optional hand-written
// glue, run after welder binds the namespace, with the backend's module handle
// named `module` in scope; write `{}` for none.
//
// `backend` selects the expansion `WELDER_DETAIL_MODULE_ENTRY_<backend>`, which
// the corresponding backend header defines (e.g. <welder/backends/pybind11.hpp>).
// Selecting a backend whose header you did not include is a preprocessor error.
