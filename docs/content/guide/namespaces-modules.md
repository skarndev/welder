...
`weld_namespace<^^ns>(m)` welds a namespace's contents *into* `m`, flat.
`weld_namespace_as_submodule<^^ns>(m)` instead first creates a **submodule** of
`m` and welds the namespace into that — the manual counterpart of what the
namespace walk above does automatically for nested namespaces, for when the
parent module is otherwise hand-written (or when you want a library's namespace
mounted under a name of your choosing).

The submodule's name defaults to the namespace's styled /
[`weld_as`](naming.md#weld_as-force-a-name-verbatim) name; an optional trailing
name is used **verbatim** and beats both. The call returns the new submodule
handle, so hand-written bindings chain right on:
